...
echo "== failed compile must preserve build/game.dll =="
SUM_BEFORE="$(sha256sum build/game.dll | awk '{print $1}')"
SIZE_BEFORE="$(wc -c < build/game.dll)"

# Deliberately break a game translation unit, attempt a module build, restore it.
cp -f src/game.c src/game.c.harness_bak
printf '%s\n' '#error deliberate harness compile failure' >> src/game.c
set +e
./build.sh
STATUS=$?
set -e
mv -f src/game.c.harness_bak src/game.c
...
SUM_AFTER="$(sha256sum build/game.dll | awk '{print $1}')"
SIZE_AFTER="$(wc -c < build/game.dll)"

if [ "$SUM_BEFORE" != "$SUM_AFTER" ] || [ "$SIZE_BEFORE" != "$SIZE_AFTER" ]; then
    echo "validate_hotreload: build/game.dll changed after failed compile" >&2
    exit 1
fi

if ls build/game_tmp.tmp >/dev/null 2>&1; then
    echo "validate_hotreload: temporary module artifact left behind" >&2
    exit 1
fi

if ls build/game_load_*.dll >/dev/null 2>&1; then
