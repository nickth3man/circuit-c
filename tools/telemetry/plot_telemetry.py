...
def chart_definitions(run: tc.Run, baseline: Optional[tc.Run] = None):
    ...
    charts = [
        ("trajectory", "Trajectory (x forward, y left)", run.column("position_x_m"),
         ([("current", run.column("position_y_m"))] +
          ([("baseline", baseline.column("position_y_m"))] if baseline is not None else [])),
         "x (m)", "y (m)", True),
        ("speed", "Speed and longitudinal velocity", time,
         series("speed_mps", "velocity_longitudinal_mps", "velocity_lateral_mps"),
         "time (s)", "m/s", False),
        ("steering_yaw", "Steering angle and yaw rate", time,
         series("steering_angle_rad", "yaw_rate_rad_s"), "time (s)", "rad, rad/s", False),
        ("sideslip", "Body sideslip", time, series("body_sideslip_rad"),
         "time (s)", "rad", False),
        ("slip_angles", "Axle slip angles", time,
         series("front_slip_angle_rad", "rear_slip_angle_rad"), "time (s)", "rad", False),
        ("slip_ratios", "Axle slip ratios", time,
         series("front_slip_ratio", "rear_slip_ratio"), "time (s)", "-", False),
        ...
        ("wheel_speed", "Wheel surface speed vs ground speed", time,
         [("front wheel surface",
         ...
         "time (s)", "m/s", False),
        ("normal_loads", "Axle normal loads", time,
         series("front_normal_load_n", "rear_normal_load_n"), "time (s)", "N", False),
        ("forces", "Limited tire forces", time,
         series("front_fx_limited_n", "rear_fx_limited_n",
                "front_fy_limited_n", "rear_fy_limited_n"), "time (s)", "N", False),
        ("friction_usage", "Friction ellipse usage", time,
         series("front_friction_usage", "rear_friction_usage"), "time (s)", "0..1", False),
        ("engine", "Engine speed and gear", time,
         series("engine_rpm", "selected_gear"), "time (s)", "rpm / gear", False),
        ("torques", "Drive and brake torques", time,
         series("drive_torque_nm", "front_brake_torque_nm", "rear_brake_torque_nm",
                "handbrake_torque_nm"), "time (s)", "N*m", False),
    ]
    ...
    if run.has("load_transfer_n"):
        charts += [
            ("axle_loads", "Axle loads: static split vs dynamic", time,
             series("static_front_load_n", "static_rear_load_n",
                    "dynamic_front_load_n", "dynamic_rear_load_n"),
             "time (s)", "N", False),
            ("load_transfer", "Longitudinal load transfer (positive = rearward)", time,
             series("load_transfer_n"), "time (s)", "N", False),
            ("accel_filter", "Acceleration filter: previous -> filtered -> solved", time,
             series("previous_long_accel_mps2", "filtered_long_accel_mps2",
                    "solved_long_accel_mps2"), "time (s)", "m/s^2", False),
            ("drag_vs_speed", "Aerodynamic drag against speed", run.column("speed_mps"),
             [("aero drag", run.column("aero_drag_n"))] +
             ([("baseline aero drag", baseline.column("aero_drag_n"))]
              if baseline is not None and baseline.has("aero_drag_n") else []),
             "speed (m/s)", "N", False),
            ("resistance", "Resistance forces over the run", time,
             series("aero_drag_n", "rolling_resistance_n"), "time (s)", "N", False),
            ("driver_inputs", "Driver inputs", time,
             series("throttle_input", "brake_input", "handbrake_input"),
             "time (s)", "0..1", False),
            ("lateral_accel", "Lateral acceleration", time,
             series("lateral_accel_mps2"), "time (s)", "m/s^2", False),
        ]
    return charts
...
def render_chart(definition) -> str:
    ...
    return tc.line_chart(title, x, series, x_label=x_label, y_label=y_label,
