...
bool telemetry_write_row(TelemetryWriter *writer, const TelemetryRow *row)
{
    ...
    const int written = fprintf(
        ...
        (double)row->engineRpm, row->selectedGear,
        ...
        (double)row->frontFxPureN, (double)row->rearFxPureN,
        (double)row->frontFyPureN, (double)row->rearFyPureN,
        (double)row->frontFxLimitedN, (double)row->rearFxLimitedN,
        (double)row->frontFyLimitedN, (double)row->rearFyLimitedN,
        (double)row->frontFrictionUsage, (double)row->rearFrictionUsage,
        row->frontLocked, row->rearLocked,
        ...
        row->stateChecksum,
        (double)row->staticFrontLoadN, (double)row->staticRearLoadN,
        (double)row->dynamicFrontLoadN, (double)row->dynamicRearLoadN,
        (double)row->loadTransferN,
        (double)row->previousLongAccelMps2, (double)row->filteredLongAccelMps2,
        (double)row->solvedLongAccelMps2, (double)row->lateralAccelMps2,
        (double)row->aeroDragN, (double)row->aeroDragXN, (double)row->aeroDragYN,
        (double)row->rollingResistanceN, (double)row->rollingResistanceXN,
        (double)row->rollingResistanceYN,
        (double)row->throttleInput, (double)row->brakeInput,
        (double)row->handbrakeInput);
    if (written < 0) {
