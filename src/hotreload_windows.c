...
#define MODULE_COPY_PREFIX "build/game_load_"
#define MODULE_COPY_GLOB   "build\\game_load_*.dll"
...
static void sweep_stale_copies(void)
{
    ...
    do {
        char path[MAX_PATH + 8];    /* room for the "build/" prefix and the terminator */
        snprintf(path, sizeof(path), "build/%s", find.cFileName);
