 ...
 * run that exercises the Phase 3 vehicle and debug HUD, writes telemetry/phase3_smoke.png,
...
int main(int argc, char **argv)
{
    ...
    while (!WindowShouldClose()) {
        ...
        if (smoke_test) {
            ...
            if (smoke_frames >= SMOKE_TEST_FRAMES) {
                TakeScreenshot("telemetry/phase3_smoke.png");
                TRACELOG(LOG_INFO,
    ...
    if (smoke_test && exit_status == 0) {
        printf("SMOKE: ok (%d frames) -> telemetry/phase3_smoke.png\n", smoke_frames);
    }
    return exit_status;
}
