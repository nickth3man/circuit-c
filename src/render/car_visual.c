 ...
 * the axle midpoint (src/physics/vehicle.h, src/dev/dev_params.c: "layout frame (axle midpoint origin)").
 ...
 *
 * Each section rises from its endpoint to the shoulder as u^exponent with the exponent BELOW
 * one, so the width change concentrates near the endpoint and the middle of the car stays near
 * its maximum. That is what the reference sheets show and what a linear ramp could not
 * reproduce: measured on 77 reference sprites, a top-down car closes ~6 px over its last 15% of
 * length and gains ~10 px over its first 15%, while the flanks between the arches are nearly
                          ...
                          float waistDepth)
{
    ...
    if (t <= ts) {
        u = t / ts;
        frac = tailFrac + (1.0f - tailFrac) * powf(u, CV_HULL_TAIL_SHAPE);
    } else {
        u = (t - ts) * (1.0f / (1.0f - ts));
...
void car_visual_derive(const VehicleSpec *spec, CarVisual *out)
{
    ...
    /* [rule] the declared per-axle flare, on top of the shared derived one: this arch's flare
     * is out->archFlareM + body.fender_flare_[front|rear]. A bolt-on flare is a real dimension
     * of the bodywork and cannot be inferred from the track — two cars with the same track and
     * tire can have flared arches or flush ones. Applied per axle, because a rear-flared car (a
    ...
    for (int i = 0; i < WHEEL_COUNT; i++) {
        ...
        out->wheels[i].archFlareM =
            out->archFlareM + (isFront ? out->fenderFlareFrontM : out->fenderFlareRearM);
    }
...
uint32_t car_visual_bake_key(const CarVisual *v)
{
    ...
    key_f32(&h, v->fenderFlareFrontM);
    key_f32(&h, v->fenderFlareRearM);
