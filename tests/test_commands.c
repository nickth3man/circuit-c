...
int test_measure_sweep(const char *key)
{
    ...
    if (stepMaps == NULL || fleetMap == NULL || stepVisuals == NULL || stepSpecs == NULL) {
        free(stepMaps); free(fleetMap); free(stepVisuals); free(stepSpecs);
        fprintf(stderr, "error: out of memory\n");
        return 1;
    }

    printf("measuring '%s' over [%g, %g] as a %d-step sweep axis at %.1f px/m\n",
           param->name, (double)param->minimum, (double)param->maximum, steps,
           (double)CV_TEST_PX_PER_M);
    printf("floors: pixels >= %.3f, L2 >= %.2f, Linf >= %.3f m\n\n",
           (double)CV_MIN_PIXEL_DIFF, (double)CV_MIN_L2, (double)CV_MIN_LINF);

    bool built = true;
    for (int s = 0; s < steps && built; s++) {
        if (!car_corpus_sweep_probe(key, s, &stepSpecs[s])) { built = false; break; }
        if (!vehicle_spec_is_valid(&stepSpecs[s])) {
            printf("step %d: value %g is not a valid spec — the axis cannot be swept here\n",
                   s, (double)dev_param_get(&stepSpecs[s], param));
            built = false;
            break;
        }
        car_visual_derive(&stepSpecs[s], &stepVisuals[s]);
        built = car_raster_draw_labels(&stepVisuals[s], canvas,
                                       stepMaps + pixels * (size_t)s, pixels);
    }
    if (!built) {
        free(stepMaps); free(fleetMap); free(stepVisuals); free(stepSpecs);
    ...
    printf("%-6s %-12s %-9s %-8s %-9s %s\n",
           "pair", "value", "pixels", "L2", "Linf(m)", "worst component");
    bool pass = true;

    /* Adjacent steps first: the tightest pairs, and the ones that kill most candidates. */
    for (int s = 1; s < steps; s++) {
        int worst = 0;
        const float d = car_raster_difference(stepMaps + pixels * (size_t)(s - 1),
                                             stepMaps + pixels * (size_t)s,
                                             canvas.width, canvas.height);
        const float linf = test_car_signature_linf(&stepVisuals[s - 1], &stepVisuals[s], &worst);
        ...
        printf("%d-%d    %-12.4g %-9.4f %-8.4f %-9.4f %s%s\n", s - 1, s,
               (double)dev_param_get(&stepSpecs[s], param), (double)d, (double)l2,
               (double)linf, car_visual_signature_component_name(worst), ok ? "" : "   FAIL");
    }
    ...
    printf("\n%-6s %-9s %-30s %-8s %-30s %-9s %s\n", "step", "pixels", "worst pixel pair",
           "L2", "worst L2 pair", "Linf(m)", "worst Linf pair");
    for (int s = 0; s < steps; s++) {
        ...
        for (int i = 0; i < fleet; i++) {
            ...
            if (d < minDiff)    { minDiff = d;    diffIndex = i; }
            if (l2 < minL2)     { minL2 = l2;     l2Index = i; }
            if (linf < minLinf) { minLinf = linf; linfIndex = i; }
        }
        ...
        const bool ok = (minDiff >= CV_MIN_PIXEL_DIFF && minL2 >= CV_MIN_L2 &&
                         minLinf >= CV_MIN_LINF);
        pass = pass && ok;
